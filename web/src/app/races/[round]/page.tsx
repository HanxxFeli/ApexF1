...
export default function RaceDetailPage({
  ...
  const race = racesMock.find(
    (r) => r.round === round && (year ? r.year === year : true)
  );
    ...
    return (
      <main className="mx-auto max-w-6xl px-5 pb-16">
        <header className="mt-8">
          <Link href="/races" className="text-red-400 hover:text-red-300 font-medium">
            ← Back to races
          </Link>
        </header>
        <section className="mt-6">
          <EmptyState title="Race not found" description="Try going back and selecting a race again." />
        </section>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto max-w-6xl px-5 pb-16">
      <header className="mt-8 flex items-start justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-widest text-white/55">Race Details</p>
      ...
      <section className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-5">
        <Card className="p-5 lg:col-span-2">
          ...
          <div className="mt-5 rounded-xl border border-white/10 bg-white/[0.03] p-4 text-sm text-white/60">
            Replace these placeholders later with: qualifying table, results table, and predictions.
          </div>
        </Card>
